...
```
#### d)
We calculated p_estimate (estimated mean of probability of getting weight under 2600) by sampling. Next, we calculated upper bound of the Confidence Interval.
```{r}
n = 100
p_lower = 0.25
...
m = p_estimate - p_lower;m 
p_upper = p_estimate + m
c(p_lower, p_estimate, p_upper)

z_alpha = m/s;z_alpha#I think this is t-value, because we don't have population standart deviation, if it is z- than this test was done with 98% confidence
```
#### e)
...
```
e)
Since here we are dealing with proportions, we decided to test whether there is difference in proportions between female and male newborns to weight more than 2600 with hypothesizes formulated as: $H_0: Pr_{female >2600} = Pr_{male>2600}$ $H_1: Pr_{female >2600} \ne Pr_{male>2600}$. However, before proportions test, we have to test whether probability for newborn to be male is equal to 0.5 with binomial test hypothesizes:$H_0: Pr(x=male)=0.5$, $H_1: Pr(x=male)\ne 0.5$. The p-value of binomial test was computed to be 0.942, which fails to reject $H_0: Pr(x=male)=0.5$. Thus, with knowing that probability for newborn to be male or female is 0.5, we can infer proportions test with computed p-value of 0.501, which also fails to reject $H_0: Pr_{female >2600} = Pr_{male>2600}$. To conclude, we disagree with expert's claim that the mean weight is different for male and female babies, because we have confirmed that $Pr(x=male)=0.5$ and the $Pr_{female Weight >2600} = Pr_{male Weight>2600}$.
```{r}
male_less2600 = 34
female_less2600 = 28
...
gender_pr_test = binom.test(x=male_above2600+male_less2600, nrow(df))
proportions_test = prop.test(c(male_above2600, female_above2600),c(male_above2600+male_less2600,female_above2600+female_less2600))
paste("p-value for testing that probability to be born female/male is equal to 0.5:", round(gender_pr_test$p.value, 3))
paste("p-value for testing whether proportions of female and male birth weights above 2600 are equal:",round(proportions_test$p.value, 3))
```
...
qqnorm(df$After8weeks, col='red', pch=19, main="Normal Q-Q plot (after 8 weeks)", font.main=10)
...
plot(x=thetas, y = p_values, type = "S", xlab = "Theta values", ylab="p-value", main = "Theta distribution according to p-values", font.main=10)
...
par(mfrow=c(1, 3))
hist(data$preweight - data$weight6weeks, breaks=10, font.main=10, main="Weight difference distribution", xlab="weight difference")
...
```
#### d)
First, we constructed 3 ANOVA models (model_a, model_b, model_c), where we included all additives and in each model we checked the interaction between additive and block. Since we found that there is no interaction between block and the additives, we computed ANOVA with no interaction (additive model). In additive model (model_d) we see that additives N and K, and also block had p-value<0.05, which allow us to state that N, K and block do have an effect on yield. Also, we have noticed that potassium have no significant effect on yield, since its p-value in all 4 models was larger than 0.05. 
```{r}
df$K = as.factor(df$K)
df$P = as.factor(df$P)
...
model_d = lm(yield ~ N + K + P + block, data=df); anova(model_d) # we should use this
#model_e = lm(yield ~ N + K + block, data=df);anova(model_e)
```

### e)
```{r}
library(lme4)
yieldlmer=lmer(yield~N+P+K+(1|block),REML=FALSE,data=df); summary(yieldlmer)
